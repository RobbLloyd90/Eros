...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  return (
    <motion.div initial={
      { opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} onClick={() => { 
      if (isEditMode) setIsEditMode(false); }} style={{ padding: '0 16px 24px 16px', display: 'flex', flexDirection: 'column', gap: '16px', height: '100%', overflowY: 'auto' }
      }>
      ...
      <motion.div 
      whileTap={{ scale: 0.98 }} onClick={onNavigateToYear} style={
        {
          ...tStyle.row,
          padding: '12px 16px',
          display: 'flex',
          ...
          cursor: 'pointer',
          width: '100%',
          backgroundColor: isLight ? 'rgba(255,255,255,0.7)' : 'rgba(255,255,255,0.03)'
        }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <Layers size={14} color={tStyle.colors.primary} />
          <span style={
            {
              fontSize: '11px',
              fontWeight: 'bold',
        ...
        <span style={
          {
            fontSize: '9px',
            color: tStyle.colors.secondary,
            fontFamily: theme.includes('nothing') ? "'DotGothic16', sans-serif" : (isLight ? 'inherit' : "'Share Tech Mono', monospace"), fontWeight: 'bold' }}
            >
      ...
      <div style={{ display: 'flex', gap: '12px', width: '100%' }}>
        <motion.div whileTap={{ scale: 0.96 }} onClick={onNavigateToCurrentMonth} style={{ ...containerStyle, flex: 1, aspectRatio: '1', cursor: 'pointer' }}>
          ...
          <div>
            <div style={{ fontSize: '10px', color: tStyle.colors.secondary, letterSpacing: '2px', fontFamily: theme.includes('nothing') ? "'DotGothic16', sans-serif" : 'inherit' }}>
              CURRENT CYCLE
            </div>
            ...
            <div style={{ fontSize: '20px', fontWeight: 'bold', color: tStyle.colors.primary, fontFamily: theme.includes('nothing') ? "'DotGothic16', sans-serif" : 'inherit' }}>
              {MONTH_NAMES[currentMonth - 1]} '{String(currentYear).slice(-2)}
            </div>
            
            <div style={{ fontSize: '10px', color: tStyle.colors.secondary, fontFamily: theme.includes('nothing') ? "'DotGothic16', sans-serif" : (isLight ? 'inherit' : "'Share Tech Mono', monospace"), marginTop: '8px', backgroundColor: tStyle.colors.metricBg, padding: '4px 6px', borderRadius: '4px', display: 'inline-block' }}>
              {quickSummary}
            </div>
          </div>
        </motion.div>
