...
export const CustomLineChart: React.FC<CustomLineChartProps> = ({ data, theme, tStyle, isLight }) => {
  ...
  if (!data || data.labels.length === 0 || data.datasets.length === 0) {
    return <div style={{ height: "150px", display: "flex", alignItems: "center", justifyContent: "center", color: tStyle.colors.secondary, fontSize: "11px", fontFamily: getLabelFontFamily(theme) }}>INSUFFICIENT HISTORICAL DATA</div>;
  }
  ...
  if (allValues.length === 0) {
    return <div style={{ height: "150px", display: "flex", alignItems: "center", justifyContent: "center", color: tStyle.colors.secondary, fontSize: "11px", fontFamily: getLabelFontFamily(theme) }}>NO DATA AVAILABLE</div>;
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginTop: "12px", width: "100%" }}>
      ...
      <div style={{ width: "100%", height: "120px", position: "relative" }}>
        <svg viewBox={`0 -10 ${chartWidth} ${chartHeight + 20}`} style={{ width: "100%", height: "100%", overflow: "visible" }} preserveAspectRatio="none">
          ...
          {data.targetValue !== undefined && data.targetValue > 0 && (
            <g>
              ...
              <text x={chartWidth - 2} y={getY(data.targetValue) - 4} fontSize="8" fill={tStyle.colors.pos} textAnchor="end" fontWeight="bold" fontFamily={getLabelFontFamily(theme)}>
            ...
            return (
              <g key={dataset.label}>
                <polyline
                  ...
                  strokeWidth={isNothingTheme(theme) ? "2" : "3"}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", position: "absolute", bottom: "-15px", left: 0, right: 0 }}>
          {data.labels.map((label, i) => (
            <span key={i} style={{ fontSize: "8px", color: tStyle.colors.secondary, fontWeight: "bold", fontFamily: getLabelFontFamily(theme) }}>
              {label}
            </span>
          ))}
        </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "12px", marginTop: "16px", justifyContent: "center" }}>
        {data.datasets.map((dataset) => (
          <div key={dataset.label} style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "9px", color: tStyle.colors.secondary, fontWeight: "bold", fontFamily: getLabelFontFamily(theme) }}>
